...
export default function ElegirListaModal({ isOpen, onClose, listas = {}, bookId, onAdded }) {
  ...
  const handleAdd = async (listName) => {
    if (!localStorage.getItem('token')) {
      toast?.push('Debes iniciar sesión', 'error');
      return;
    }
    ...
    try {
      ...
      toast?.push(error.response?.data?.error || error.response?.data?.message || 'No se pudo agregar el libro', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRemove = async (listName) => {
    ...
    try {
      ...
      toast?.push(error.response?.data?.error || 'No se pudo quitar el libro de la lista', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateAndAdd = async () => {
    ...
    try {
      ...
      toast?.push(error.response?.data?.error || 'No se pudo crear la lista', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card lists-modal" onClick={(event) => event.stopPropagation()}>
        <h3>Selecciona una lista</h3>
        {showCreate ? (
          <div className="list-create-inline">
            <input value={newListName} onChange={(event) => setNewListName(event.target.value.slice(0, 50))} maxLength={50} placeholder="Nombre de la lista" autoFocus />
            <span className="list-name-counter">{newListName.length}/50</span>
            <div className="modal-actions">
              <button className="btn" type="button" onClick={() => setShowCreate(false)}>Cancelar</button>
              <button className="btn-primary" type="button" onClick={handleCreateAndAdd} disabled={loading}>{loading ? 'Creando...' : 'Crear y agregar'}</button>
            </div>
          </div>
        ) : (
          <>

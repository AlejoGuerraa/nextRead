...
const enviarSolicitudSeguimiento = async (req, res) => {
    try {
        ...
        const existente = await Seguidos.findOne({ where: { id_remitente: remitenteId, id_destinatario: targetId } });
            ...
            await agregarNotificacion(targetId, `${remitente.usuario} te ha vuelto a enviar una solicitud para seguirte.`, remitente.usuario, { type: 'follow_request', requestId: existente.id, fromUser: remitenteId, status: 'enviado' });
        ...
        const remitente = await User.findByPk(remitenteId);
        await agregarNotificacion(targetId, `${remitente.usuario} te ha enviado una solicitud para seguirte.`, remitente.usuario, { type: 'follow_request', requestId: registro.id, fromUser: remitenteId, status: 'enviado' });
...
const responderSolicitud = async (req, res) => {
    try {
        ...
        // --- Actualizar la notificación original en el destinatario ---
        // Para evitar que queden notificaciones antiguas con status 'enviado' (que muestran botones),
        // eliminamos cualquier notificación existente con el mismo requestId y añadimos
        // una nueva notificación clara con `meta.status` actualizado.
        try {
            ...
            if (destinatario) {
                const originalRaw = destinatario.notificaciones;
                let notis = parseNotis(originalRaw);

                const reqId = Number(solicitud.id);

                // Filtrar cualquier notificación antigua con el mismo requestId
                const filtered = notis.filter(n => !(n && n.meta && Number(n.meta.requestId) === reqId));

                // Construir la nueva notificación para el destinatario (quien respondió)
                const nombreRemitente = remitente ? (remitente.usuario || `${remitente.nombre || ''}`) : 'usuario';
                const nuevaNoti = {
                    id: Date.now(),
                    nombre: destinatario.usuario || destinatario.nombre || 'Tú',
                    mensaje: solicitud.estado === 'aceptado' ? `Has aceptado la solicitud de ${nombreRemitente}.` : `Has rechazado la solicitud.`,
                    fecha: new Date().toISOString(),
                    meta: { type: 'follow_request', status: solicitud.estado, requestId: solicitud.id, fromUser: destinatario.id }
                };

                // Añadir al principio
                filtered.unshift(nuevaNoti);

                destinatario.notificaciones = filtered;
                await destinatario.save();
                console.log('✅ Notificaciones del destinatario reemplazadas y añadida nueva notificación:', { userId: destinatario.id, requestId: solicitud.id, nuevoMeta: nuevaNoti.meta });
            }
        } catch (e) {
            console.error('Error actualizando notificaciones del destinatario:', e);
        }

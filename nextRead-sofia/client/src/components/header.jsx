...
export default function Header({ user, onRestrictedAction, headerRightRef }) {
  ...
  const fetchCurrentUser = async () => {
    try {
      ...
      const resp = await axios.get('http://localhost:3000/nextread/user', { headers: { Authorization: `Bearer ${token}` } });
      return resp.data;
    } catch (err) {
  ...
  const handleIconClick = (target) => {
    if (currentUser) {
      if (target === "notificaciones") {
        // Antes de abrir el modal, traemos el usuario completo (con notificaciones)
  ...
  return (
    <>
      ...
      <NotificacionesModal
        ...
        data={notifData}
        onRefresh={async () => {
          // Cerrar modal, refrescar datos, y reabrirlo
          setOpenNotif(false);
          const u = await fetchCurrentUser();
          setNotifData(u);
          // Pequeña pausa para asegurar que React renderiza el cierre
          setTimeout(() => {
            setOpenNotif(true);
          }, 100);
        }}
